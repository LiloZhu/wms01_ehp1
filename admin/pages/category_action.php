...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    switch($_POST['action']) {
        case 'add':
            $sql = "insert into tb_category values(null,'{$data['cat_code']}','{$data['cat_text']}','{$data['where_use']}','{$data['seq']}'); ";
            $action_flag = $ado->Create($sql);
            
            if ($action_flag > 0){
                $arrResult['success']= true;
                $arrResult['message']= $data['cat_code'];
            }
            break;
        case 'edit':
            $sql = "update tb_category set cat_code = '{$data['cat_code']}', cat_text = '{$data['cat_text']}', where_use = '{$data['where_use']}', seq = '{$data['seq']}' where id = '{$data['id']}'; ";
            $action_flag = $ado->Create($sql);
            if ($action_flag > 0){
                $arrResult['success']= true;
            ...
            break;
        case 'delete':
            $sql = "delete from tb_category where id = '{$data['key']}'; ";
