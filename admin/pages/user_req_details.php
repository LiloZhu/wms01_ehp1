...
<div id="userRequestModalContainer"></div>
    ...
    initialize: function(){
            this.set({
                    ...
                    
                {
                    id: 'btn_rel',
				    text: '批准',
				    data_target: '#modal_rel',
				    class:'btn top btn-primary btn_modal toolbar_action',
                },
                {
                    id: 'btn_rej',
				    text: '拒绝',
				    data_target: '#modal_rej',
				    class:'btn top btn-danger btn_modal toolbar_action',
                },                
                {
                	id: 'btn_edit',
   				 	text: '编辑',
...
toolbarModalREL = new toolbarModalModel({id:'modal_rel',title:'批准'});
toolbarModalREJ = new toolbarModalModel({id:'modal_rej',title:'拒绝'});
...
var userRequestData = new userRequestModel({data:{url: 'user_req_details_action.php', 
	action:'retrieve', 
		arguments:{ 	
...
function ajaxRequest(params){
	...
	//debugger;
	$.ajax({
		url: "user_req_details_action.php",
		type: "POST",
		dataType: "json",
...
//<-Edit  
  ...
  $('#btn_rel').click(function(){
		...
		layer.confirm('您是否要批准当前 ' + ids.length + '条数据？', {
		btn: ['是', '否']
		}, function() {
			relServer(ids,'release');
		});
	});

  $('#btn_rej').click(function(){
		var ids = getSelections();
		layer.confirm('您是否要拒绝当前 ' + ids.length + '条数据？', {
		btn: ['是', '否']
		}, function() {
			relServer(ids,'reject');
		});
	});	

	//批准 / 拒绝
	function relServer(ids,act){
		//->
	var request=new XMLHttpRequest;
	           $.ajax({
	            type:"POST",
...
function displaycolor(value,row,index) {
	var a = "";
	if(value == "NEW") {
		var a = '<span style="color:blue">待审批</span>';
	}else if(value == "REL"){
		var a = '<span style="color:green">已批准</span>';
	}
	else if(value == "REJ"){
		var a = '<span style="color:red">已拒绝</span>';	
	}else{
...
$scope.initSearchData=function(){
	...
	//$scope.params = {"action": "userRequest_get"};
	...
	$http({
      ...
      //$localStorage.userRequestdata = JSON.stringify(data.data.rows);
      ...
      $scope.initcompanyData();
      //sessionStorage.setItem("userRequestlist",$scope.userRequestdata);
	}
...
$scope.initcompanyData=function(){
	...
	/*
	for (var i = 0; i < $scope.userRequestdata.length; i++) {
		 $("#searchuserRequestCode").append('<option  value="' + item + '">' + item + '</option>');
	}	
	*/
...
$("#searchcompanyCode").on('changed.bs.select',
      function (e) {
      //alert($('#searchuserRequestCode').val());
      $table.bootstrapTable('refresh');
      	//SearchRequest();
      });
