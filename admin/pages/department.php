...
               }
    ...
               $.ajax({
    	...
    	               }
    				   ...
    				   window.location='department.php' + condition;
    	           	 },
						error:function(data){
...
$('#btn_edit').click(function(){
    ...
    $('' + action + 'department_text').val(getSelectedRowFieldValue('#tab_01','department_text'));
...
$('.table_action_edit').click(function(){
    ...
    $('' + action + 'department_code').val(row.eq(2).text());
    $('' + action + 'department_text').val(row.eq(3).text());
    $('' + action + 'company_code').val(row.eq(4).text());
    $('' + action + 'active').attr("checked",(row.eq(5).text())== 1 ? true : false );
    $('' + action + 'delete_flag').attr("checked",(row.eq(6).text()) == 1 ? true : false );
...
function eventregister_button($_action){
    ...
    $department_code = isset($_GET['{$_action}_department_code']) ? $_GET['{$action}department_code'] : "";
    $department_text = isset($_GET['{$_action}department_text']) ? $_GET['{$_action}department_text'] : "";
    $conpany_code = isset($_GET['{$_action}_company_code']) ? $_GET['{$_action}company_code'] : "";
    ...
    //--->
    $_body .= " <div class='form-inline'>";
    $_body .= " <label for='{$_action}department_code' class='control-label mr-1'>代码</label>";
    $_body .= " <input type='text' class='form-control' style='width:120px' id='{$_action}department_code' name='{$_action}department_code' value='{$department_code}' placeholder='请输入代码'>";
    $_body .= " </div>";
    //<---
    //--->
    $_body .= " <div class='form-inline'>";
    $_body .= " <label for='{$_action}department_text' class='control-label mr-1'>名称</label>";
    $_body .= " <input type='text' class='form-control' style='width:400px' id='{$_action}department_text' name='{$_action}department_code' value='{$department_text}' placeholder='请输入名称'>";
    $_body .= " </div>";
    //<---
    $_body .="</div>";
    //<-
...
function build_table(){
    $_array_tag =[
        "TABLE" => [
            ...
            "PAGING_ENABLE" => true,
            "PAGING_FORMAT" => "2",
            "SQL" =>"select * from tb_department where delete_flag = false",
            "SQL_HAS_WHERE"=> true,
        ],
        "THEAD" => [
            "CLASS" => ""
