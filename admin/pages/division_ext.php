...
function getCompany(){
	if(sessionStorage.getItem("companylist") == null || sessionStorage.getItem("companylist") == '' || sessionStorage.getItem("companylist") == 'undefined') {
	var company;
	$.ajax({
		...
		company = JSON.parse(sessionStorage.getItem("companylist"))
	}
   return company;
};

function getDepartment(){
	if(sessionStorage.getItem("departmentlist") == null || sessionStorage.getItem("departmentlist") == '' || sessionStorage.getItem("departmentlist") == 'undefined' ) {
	var department;
	$.ajax({
		url: "common_action.php",
	    type:"POST",
		...
		success: function(data){
			...
			sessionStorage.setItem("departmentlist",JSON.stringify(data.rows));
			//debugger;
		},
		error: function(data){
		}
		...
		department = JSON.parse(sessionStorage.getItem("departmentlist"))
	}
   return department;
};

function getDepartmentByCompanyCode(vCompanyCode){
	var department;
	$.ajax({
		url: "common_action.php",
	    type:"POST",
	    ...
	    data:{action:'special_select', 
  				  arguments:{
  					...
  					field_text: 'department_text',
  					link_table: 'tb_department',
  					clause_1 :   "and company_code ='" + vCompanyCode + "'" ,
			      }
				},				
		success: function(data){
			department = data.rows;
...
<div id="divisionModalContainer"></div>
...
<div class="modal fade" id="{{this.attributes.id}}"  tabindex="-1" role='dialog' aria-labelledby="{{this.attributes.id}}Label">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    ...
    <div class='form-inline'>
    <label for='{{this.attributes.id}}division_code' class='control-label mr-1'>代码</label>
    <input type='text' class='form-control' style='width:120px' id='{{this.attributes.id}}division_code' name='{{this.attributes.id}}division_code'>
    </div>
    
    
    <div class='form-inline'>
    <label for='{{this.attributes.id}}division_text' class='control-label mr-1'>名称</label>
    <input type='text' class='form-control' style='width:400px' id='{{this.attributes.id}}division_text' name='{{this.attributes.id}}division_text'>
    </div>
    ...
    <div class='form-inline'>
    <label for='{{this.attributes.id}}company' class='control-label mr-1'>公司</label>
    <select class="selectpicker" id="{{this.attributes.id}}company" data-width="480px" data-live-search="true" title="请选择" data-live-search-placeholder="搜索" >
        <option data-tokens="---ALL---"></option>
        {{#each this.attributes.company}}
...
<script type="text/template" id="company_ddl_template">
   {{#with this}}
   {{#checkIf this.attributes.id '==' 'modal_new' '||' this.attributes.id '==' 'modal_edit' }}
    <div class='form-row mb-1'>
    <div class='form-inline'>
...
<script type="text/template" id="department_ddl_template">
   {{#with this}}
   {{#checkIf this.attributes.id '==' 'modal_new' '||' this.attributes.id '==' 'modal_edit' }}
    <div class='form-row mb-1'>
    <div class='form-inline'>
    <label for='{{this.attributes.id}}department' class='control-label mr-1'>部门</label>
    <select class="selectpicker" id="{{this.attributes.id}}department" data-width="480px" data-live-search="true" title="请选择" data-live-search-placeholder="搜索" >
        <option data-tokens="---ALL---"></option>
        {{#each this.attributes.department}}
        <option value='{{code}}'>{{text}}</option>
        {{/each}}
    </select>
    </div>
...
<script type="text/template" id="division_modal_template">
{{#with this}}
...
toolbarModalNew.attributes.company = getCompany();
toolbarModalEdit.attributes.company = getCompany();
toolbarModalNew.attributes.department = getDepartment();
toolbarModalEdit.attributes.department = getDepartment();
...
toolbarModalView = Backbone.View.extend({
  ...
  template_ext1: Handlebars.compile($('#company_ddl_template').html()),
  template_ext2: Handlebars.compile($('#department_ddl_template').html()),
  
  render: function(){ 
  	Handlebars.registerPartial('company',this.template_ext1);
  	Handlebars.registerPartial('department',this.template_ext2);
  ...
  events: {
      ...
      'changed.bs.select #modal_newcompany': 'modal_new_company_handle',
      'changed.bs.select #modal_newdepartment': 'modal_new_department_handle',
      'changed.bs.select #modal_editcompany': 'modal_edit_company_handle',
      'changed.bs.select #modal_editepartment': 'modal_edit_department_handle'             
  }, 
  modal_new_submit_handle: function(event){
  	   //alert('modal submit')
  },
  modal_new_company_handle: function(event){
	  if($('#modal_newcompany').val() == '' && $('#modal_newdepartment').val() != ''){
		  toastr.options.positionClass = 'toast-top-center';  
		  toastr.error("请先选择公司后再选择部门!");  
	  }
	  if (event.target.value != '' ){
		  //alert(event.target.value);
		  department = getDepartmentByCompanyCode(event.target.value);
		  $('#modal_newdepartment').empty();
		  $('#modal_newdepartment').selectpicker('render');
		  $.each(department, function (i, item) {
				$("#modal_newdepartment").append('<option  value="' + item.code + '">' + item.text + '</option>');
			});
		  $('#modal_newdepartment').selectpicker('refresh');
		  $('#modal_newdepartment').selectpicker('render');
		  
	  }else{
		  $('#modal_newdepartment').val(''); 
		  $('#modal_newdepartment').empty();
		  $('#modal_newdepartment').selectpicker('refresh');
		  $('#modal_newdepartment').selectpicker('render');	 		  
	  }
  },
  modal_new_department_handle: function(event){
	  if($('#modal_newcompany').val() == '' && $('#modal_newdepartment').val() != ''){
		  toastr.options.positionClass = 'toast-top-center';  
		  toastr.error("请先选择公司后再选择部门!"); 
		  $('#modal_newdepartment').val(''); 
  ...
  modal_edit_company_handle: function(event){
	  if($('#modal_editcompany').val() == '' && $('#modal_editdepartment').val() != ''){
		  toastr.options.positionClass = 'toast-top-center';  
		  toastr.error("请先选择公司后再选择部门!");  
	  }
	  if (event.target.value != '' ){
		  //alert(event.target.value);
		  department = getDepartmentByCompanyCode(event.target.value);
		  $('#modal_editdepartment').empty();
		  $('#modal_editdepartment').selectpicker('render');
		  $.each(department, function (i, item) {
				$("#modal_editdepartment").append('<option  value="' + item.code + '">' + item.text + '</option>');
			});
		  $('#modal_editdepartment').selectpicker('refresh');
		  $('#modal_editdepartment').selectpicker('render');
		  
	  }else{
		  $('#modal_editdepartment').val(''); 
		  $('#modal_editdepartment').empty();
		  $('#modal_editdepartment').selectpicker('refresh');
		  $('#modal_editdepartment').selectpicker('render');	 		  
	  }	  
  },
  modal_edit_department_handle: function(event){
	  if($('#modal_editcompany').val() == '' && $('#modal_editdepartment').val() != ''){
		  toastr.options.positionClass = 'toast-top-center';  
		  toastr.error("请先选择公司后再选择部门!"); 
		  $('#modal_editdepartment').val(''); 
...
var divisionData = new divisionModel({data:{url: 'division_ext_action.php', 
	action:'retrieve', 
		arguments:{ 	
...
$("#btn_edit").click(function(){
	...
	{	
		...
		$('' + action + 'division_text').val( rows[0].division_text);
		$('' + action + 'department').val( rows[0].department_code);
		$('' + action + 'department').selectpicker('render');	
		$('' + action + 'company').val( rows[0].company_code);	
		$('' + action + 'company').selectpicker('render');
		$(action).modal('toggle');
...
var department_code = $('' + action + 'department').val();
var company_code = $('' + action + 'company').val();
...
$scope.initSearchData=function(){
	...
	//$scope.params = {"action": "division_get"};
	...
	$http({
      ...
      //$localStorage.divisiondata = JSON.stringify(data.data.rows);
      ...
      $scope.initcompanyData();
      //sessionStorage.setItem("divisionlist",$scope.divisiondata);
	}
...
$scope.initcompanyData=function(){
	...
	for (var i = 0; i < $scope.divisiondata.length; i++) {
		 $("#searchdivisionCode").append('<option  value="' + item + '">' + item + '</option>');
	}	
	*/
