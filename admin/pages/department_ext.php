...
<div id="departmentModalContainer"></div>
...
<div class="modal fade" id="{{this.attributes.id}}"  tabindex="-1" role='dialog' aria-labelledby="{{this.attributes.id}}Label">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    ...
    <div class='form-inline'>
    <label for='{{this.attributes.id}}department_code' class='control-label mr-1'>代码</label>
    <input type='text' class='form-control' style='width:120px' id='{{this.attributes.id}}department_code' name='{{this.attributes.id}}department_code'>
    </div>
    
    
    <div class='form-inline'>
    <label for='{{this.attributes.id}}department_text' class='control-label mr-1'>名称</label>
    <input type='text' class='form-control' style='width:400px' id='{{this.attributes.id}}department_text' name='{{this.attributes.id}}department_text'>
    </div>
  ...
  {{>company}}
    ...
    <div id ='{{this.attributes.id}}aaa'></div>
...
<script type="text/template" id="department_modal_template">
{{#with this}}
...
var departmentData = new departmentModel({data:{url: 'department_ext_action.php', 
	action:'retrieve', 
		arguments:{ 	
...
$("#btn_edit").click(function(){
	...
	{	
		...
		$('' + action + 'department_text').val( rows[0].department_text);
...
var department_text = $('' + action + 'department_text').val();
...
$scope.initSearchData=function(){
	...
	if(sessionStorage.getItem("companylist") == null || sessionStorage.getItem("companylist") == '') {
	//$scope.params = {"action": "department_get"};
	...
	$http({
      ...
      //$localStorage.departmentdata = JSON.stringify(data.data.rows);
      ...
      $scope.initcompanyData();
      //sessionStorage.setItem("departmentlist",$scope.departmentdata);
	}
...
$scope.initcompanyData=function(){
	...
	/*
	for (var i = 0; i < $scope.departmentdata.length; i++) {
		 $("#searchdepartmentCode").append('<option  value="' + item + '">' + item + '</option>');
	}	
	*/
...
$("#searchcompanyCode").on('changed.bs.select',
      function (e) {
      //alert($('#searchdepartmentCode').val());
      $table.bootstrapTable('refresh');
      	//SearchRequest();
      });
