...
<div id="toolbar" class="btn-group">
	<button class="btn btn-outline-primary" id="release"><i class="fas fa-user-check"></i>批准入库</button>
</div>
	...
	<table id="table" class="table-sm" data-toolbar="#toolbar"
		...
		<thead style="background: #efefef;">
			<tr>
				...
				-->
				<th data-field="inbound_status_text" data-formatter="displaycolor" >状态</th>
 ...
 layui.use('layer', function(){ //独立版的layer无需执行这一句
  var layer = layui.layer; //独立版的layer无需执行这一句
	...
	$release.on('click', function(){
		...
		layer.confirm('您是否要确认当前交货单 [' + row[0].order_number + '] 的入库？', {
		btn: ['是', '否']
		}, function() {
			release(row);
			//releaseMail(row,'REL');
...
            }
	...
	function displaycolor(value,row,index) {
		var a = "";
		if(value == "待入库") {
		var a = '<span style="color:blue;font-weight:700;">'+value+'</span>';
		}else if(value == "已入库"){
		var a = '<span style="color:green;font-weight:400;">'+value+'</span>';
		}else if(value == "处理中") {
