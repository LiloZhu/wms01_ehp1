...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    switch($_POST['action']) {
        ...
        case 'retrieve':
            $sql = "";
            $sql = "select * from v_cart_text where create_by = '{$uid}' and status_code = 'NEW' ; ";
        ...
        case 'realtime_check':
            $sql = "";
            $sql = "select * 
                    from v_cart_text where create_by = '{$uid}' 
                    and status_code = 'NEW'
                    and (rem_label_qty < label_qty or rem_label_qty <= 0);";
            
            $data=$ado->Retrieve($sql);
            
            $arrResult['rows'] = $data;
            $arrResult['noShelf'] = false;
            if ($data != false){
                if (count($arrResult['rows']) > 0) {
                    $arrResult['noShelf'] = true;
                }else{
                    $arrResult['noShelf'] = false;
                }
            }
        ...
        case 'cart_transport':
            ...
            $sql = "update tb_cart set status_code = 'REL',change_at = sysdate(), change_by = '{$uid}' where id = '{$data['id']}'; ";
            $action_flag = $ado->Create($sql);
        ...
        case 'edit':
            $sql = "";
            $sql = "update tb_cart set label_qty = '{$data['label_qty']}',qty = '{$data['qty']}',
            ...
            break;
        case 'delete':
            $sql = "update tb_cart set delete_flag = true,change_at = sysdate(), change_by = '{$uid}' where id = '{$data['key']}'; ";
